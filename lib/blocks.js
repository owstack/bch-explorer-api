...
BlockController.prototype._normalizePrevHash = function(hash) {
  // TODO fix bcccore to give back null instead of null hash
...
BlockController.prototype._getBlockSummary = function(hash, moreTimestamp, next) {
  ...
  if (summaryCache) {
    ...
    self.node.services.bitcoind.getRawBlock(hash, function(err, blockBuffer) {
      ...
      var br = new bcccore.encoding.BufferReader(blockBuffer);
      ...
      // would require changes in bcccore-node.
      var header = bcccore.BlockHeader.fromBufferReader(br);
      var info = {};
      var txlength = br.readVarintNum();
      info.transactions = [bcccore.Transaction().fromBufferReader(br)];
