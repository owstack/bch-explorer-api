...
BlockController.prototype._normalizePrevHash = function(hash) {
  // TODO fix bchLib to give back null instead of null hash
...
BlockController.prototype._getBlockSummary = function(hash, moreTimestamp, next) {
  ...
  if (summaryCache) {
    ...
    self.node.services.bitcoind.getRawBlock(hash, function(err, blockBuffer) {
      ...
      var br = new bchLib.encoding.BufferReader(blockBuffer);
      ...
      // would require changes in bch-node.
      var header = bchLib.BlockHeader.fromBufferReader(br);
      var info = {};
      var txlength = br.readVarintNum();
      info.transactions = [bchLib.Transaction().fromBufferReader(br)];
