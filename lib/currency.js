...
CurrencyController.prototype.index = function(req, res) {
  ...
  if (self.rates['USD'] === undefined || currentTime >= (self.timestamp + self.currencyDelay)) {
    ...
    request(rateSources['USD'].url, function(err, response, body) {
      if (err) {
      ...
      if (!err && response.statusCode === 200) {
        ...
        self.rates['USD'] = {
          name: rateSources['USD'].name,
          rate: parseFloat(_.get(JSON.parse(body)[rateSources['USD'], rateSources['USD'].path]))
        };
      }
      res.jsonp({
        status: 200,
