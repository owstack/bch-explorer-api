...
AddressController.prototype.getAddressSummary = function(address, options, callback) {
  ...
  this.node.getAddressSummary(address, options, function(err, summary) {
    ...
    var transformed = {
      addrStr: address,
      balance: Unit.fromSatoshis(summary.balance).toBCH(),
      balanceSat: summary.balance,
      totalReceived: Unit.fromSatoshis(summary.totalReceived).toBCH(),
      totalReceivedSat: summary.totalReceived,
      totalSent: Unit.fromSatoshis(summary.totalSpent).toBCH(),
      totalSentSat: summary.totalSpent,
      unconfirmedBalance: Unit.fromSatoshis(summary.unconfirmedBalance).toBCH(),
